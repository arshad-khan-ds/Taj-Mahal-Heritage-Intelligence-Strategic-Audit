...
import io
# Optional PDF export via reportlab (if installed)
try:
...
st.markdown("<h1 style='text-align:center;margin-top:6px;'>🏛️ THE LUSTER AUDIT</h1>", unsafe_allow_html=True)
st.markdown("<p style='text-align:center;color:#555; font-size:1.1rem;'>Strategic Impact of Air Quality on Heritage Perception & Brand Equity</p>", unsafe_allow_html=True)
...
st.image("https://images.unsplash.com/photo-1564507592333-c60657eea523?auto=format&fit=crop&w=1200&q=80", use_column_width='always')
...
try:
    ...
    def kpi_card(title, value, note, color="#ffffff"):
        card_html = f"""
        <div style='background:{color};padding:18px;border-radius:10px;box-shadow:0 10px 30px rgba(0,0,0,0.12);'>
            <div style='font-size:0.9rem;color:#444;font-weight:700;margin-bottom:6px;'>{title}</div>
            <div style='font-size:1.6rem;color:#111;font-weight:800;margin-bottom:6px;'>{value}</div>
    ...
    with col1:
        st.markdown(kpi_card("AI Brand Sentiment", f"{round(avg_sent,2)} / 5.0", f"Δ {round(delta_sent,2)} since period start", "#FFFFFF"), unsafe_allow_html=True)
    with col2:
        st.markdown(kpi_card("Visitor Star Rating", f"{round(avg_rating,2)} / 5.0", "Population-weighted average", "#FFFFFF"), unsafe_allow_html=True)
    with col3:
        st.markdown(kpi_card("Politeness Gap", f"{round(sentiment_gap,2)}", "Rating - AI sentiment", "#FFF8E1"), unsafe_allow_html=True)
    with col4:
        st.markdown(kpi_card("Total Reviews", f"{int(review_vol)}", "Aggregated review count", "#F0F7FF"), unsafe_allow_html=True)
    ...
    # Add data download button for executive summary
    try:
        csv_buffer = io.StringIO()
        display_df.to_csv(csv_buffer, index=False)
        st.download_button("Download Executive Summary as CSV", csv_buffer.getvalue(), file_name="executive_data_summary.csv", mime="text/csv")
    except Exception:
        pass
    ...
    if has_reportlab:
        try:
            pdf_bytes = generate_pdf(display_df, title="Taj Mahal Heritage Intelligence Report")
            st.download_button("Download PDF Report", pdf_bytes, file_name="taj_report.pdf", mime="application/pdf")
        except Exception:
            st.warning("PDF export failed. Ensure `reportlab` is installed.")
    else:
        st.info("Install `reportlab` to enable PDF export: `pip install reportlab` (or add it to your environment).")

    # Footer
    st.markdown("<hr style='margin:20px 0'>", unsafe_allow_html=True)
    st.markdown("<p style='text-align:center;color:#777;margin:6px 0 30px 0'>Taj Mahal Heritage Intelligence • Built for strategic insights and stakeholder decision-making</p>", unsafe_allow_html=True)
